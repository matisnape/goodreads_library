defmodule GoodreadsLibrary.Actions.Import do
  ...
  def import_to_db([]), do: :ok

  def import_to_db(remote_books) do
    ...
    db_ids_to_delete = db_books_ids -- remote_ids
    ids_to_update = db_books_ids -- db_ids_to_delete
  ...
  def insert_book_to_db(remote_id, remote_books) do
  ...
  def update_book_by_remote_id(remote_id, remote_books) do
    remote_book = remote_books |> Enum.find(fn book -> book.remote_id == remote_id end)
  ...
  def delete_book_by_remote_id(remote_id) do
    Queries.Book.get_by_remote_id(remote_id)
    |> Repo.delete()
    |> case do
      {:ok, book} ->
